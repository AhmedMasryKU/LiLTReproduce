...
from transformers.modeling_outputs import MaskedLMOutput, BaseModelOutputWithPooling, BaseModelOutput
from transformers.activations import gelu 
from transformers.models.lilt.modeling_lilt import LiltTextEmbeddings, LiltLayoutEmbeddings, LiltLayer
...
class LiltEncoder(nn.Module):
    ...
    def forward(
        ...
        # Concatenate!
        #print(hidden_states.size(), layout_inputs.size())
        # hidden_states = torch.cat([hidden_states, layout_inputs], dim=2)
        if not return_dict:
            return tuple(
                v
                for v in [
...
class LiltPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size+config.hidden_size//4, config.hidden_size)
